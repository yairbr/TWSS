module.exports = function(){
  ...
  var calc_score = function(score_obj){
  	...
  	if(!score_obj.ranking || score_obj.ranking == 0)
  		score_obj.ranking = 1
  	var score_obj.numerator = ((score_obj.scoring + 1) * score_obj.ranking * score_obj.rating);
    var denominator = (score_obj.scoring + score_obj.ranking + score_obj.rating);
    return numerator/denominator;
  }

// change here the rout
  router.post('/sort_elements', function(req, res, next) {
    var type = "_" + req.type;
    var debId = req.id;
	...
	objs.sort(function(a,b) {return (calc_score(a) > calc_score(b)) ? 1 : ((calc_score(b) > calc_score(a)) ? -1 : 0);} ); 
